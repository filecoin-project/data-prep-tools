...
if __name__ == '__main__':
    ...
    car_then_split = CarThenSplit(
        ...
        car_generator=go_car,  # IpldGoCar's generate_car function can take in a glob as data_dir
    ...
    root_cid = go_car.get_root_cid(car_file)
    os.unlink(car_file)

    f = open(metadata_file, 'w')
    writer = csv.writer(f)
    writer.writerow(["timestamp", "original data", "car file", "root_cid", "piece cid", "padded piece size"])
    car_files = car_then_split.find_car_files()
    for car_file in car_files:
        deal_params = car_then_split.calculate_deal_params(car_file)
        row = [datetime.now(), data_dir, car_file, root_cid, deal_params['commp'], deal_params['padded_piece']]
